...
const getExampleScripts = (framework?: string) => {
    return [
        {
            name: `${framework || ''}${framework ? '-framework-' : ''}script-from-package-1`,
            command: 'example_value1'
        },
        {
            name: `${framework || ''}${framework ? '-framework-' : ''}script-from-package-2`,
            command: 'example_value2',
            addInShared: true,
            addInApp: true
        },
        {
            name: `${framework || ''}${framework ? '-framework-' : ''}script-from-package-3`,
            command: 'example_value3',
            addInShared: true,
            addInApp: false
        },
        {
            name: `${framework || ''}${framework ? '-framework-' : ''}script-from-package-4`,
