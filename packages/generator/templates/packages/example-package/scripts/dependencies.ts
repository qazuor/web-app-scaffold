...
const getDependencyExample = (framework?: string, isDev?: boolean) => {
    return [
        {
            name: `${framework || ''}${framework ? '-framework-' : ''}example-${isDev ? 'dev-' : ''}dependency-from-package-1`,
            version: '^1.0.0'
        },
        {
            name: `${framework || ''}${framework ? '-framework-' : ''}example-${isDev ? 'dev-' : ''}dependency-from-package-2`,
            version: '^1.0.0',
            addInShared: true,
            addInApp: true
        },
        {
            name: `${framework || ''}${framework ? '-framework-' : ''}example-${isDev ? 'dev-' : ''}dependency-from-package-3`,
            version: '^1.0.0',
            addInShared: true,
            addInApp: false
        },
        {
            name: `${framework || ''}${framework ? '-framework-' : ''}example-${isDev ? 'dev-' : ''}dependency-from-package-4`,
            version: '^1.0.0',
            addInShared: false,
            addInApp: true
        }
