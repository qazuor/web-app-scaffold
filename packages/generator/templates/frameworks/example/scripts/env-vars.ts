...
const getEnvVarsExample = (framework?: string) => {
    return [
        {
            name: `${framework || ''}${framework ? '_FRAMEWORK_' : ''}ENVVAR_EXAMPLE_1`,
            value: 'example_value1'
        },
        {
            name: `${framework || ''}${framework ? '_FRAMEWORK_-' : ''}ENVVAR_EXAMPLE_2`,
            value: 'example_value2',
            addInShared: true,
            addInApp: true
        },
        {
            name: `${framework || ''}${framework ? '_FRAMEWORK_-' : ''}ENVVAR_EXAMPLE_3`,
            value: 'example_value3',
            addInShared: true,
            addInApp: false
        },
        {
            name: `${framework || ''}${framework ? '_FRAMEWORK_-' : ''}ENVVAR_EXAMPLE_4`,
